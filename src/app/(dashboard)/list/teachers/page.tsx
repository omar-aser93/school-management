...
const TeacherListPage = async ({searchParams} : {searchParams: {[key: string]: string | undefined }}) => {  //get url searchParams (queries)
  ...
  const Items_Per_Page = 10 ;                       //we can make it dynamic in a separete client component & sent the value through URL params
  ...
  const data = await getTeachers(query, currentPage, Items_Per_Page, sort);  
